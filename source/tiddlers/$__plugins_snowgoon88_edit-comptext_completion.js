 ...
 * </ul>
 */
    var Template = function( pat, filter, mask, field, start, end, startOffset, endOffset  ) {
    this.pat = pat;
        this.filter = filter;
	    ...
	    if( temp.body ) {		
    		this._listTemp.push( 
    		    new Template( temp.pattern, temp.body,
				  ...
				  "body",
    				  temp.start, temp.end,
                                temp.startOffset, temp.endOffset)
    		);
	    }
	    else {
    		this._listTemp.push( 
...
var insertInto = function(node, text, posBefore, posAfter, template ) {
    ...
    console.log(template.endOffset);
    if (template.startOffset) {
        let cursorPos = posBefore+template.startOffset;
        node.setSelectionRange( cursorPos, cursorPos );
    }
    else if (template.endOffset) {
        let cursorPos = posBefore+text.length+sStart.length+sEnd.length-template.endOffset;
        node.setSelectionRange( cursorPos, cursorPos );
    }
    else {
